...
class Score {
  ...
  updateDisplay() {
    ...
    currentScoreElement.innerText = `Текущий счет: ${this.currentScore} ОЧКОВ`;
    highScoreElement.innerText = `Рекорд: ${this.highScore} ОЧКОВ`;
  }
  reset() {
    this.currentScore = 0;
  }
...
class Game {
  ...
  update() {
    ...
    if (this.hitSelf() || this.snake.hitWall(this.rows, this.columns)) {
      alert('ИГРА ОКОНЧЕНА! ' + 'ВАШ СЧЕТ - ' + this.score.currentScore);
      this.score.save();
      clearInterval(this.intervalId);
      this.reset();
      return;
